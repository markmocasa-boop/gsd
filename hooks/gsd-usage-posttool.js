...
// Captures tool usage and calculates token costs for statusline display
//
// IMPORTANT: Claude Code's PostToolUse hook may not provide actual token counts
// in the data it passes to hooks. This implementation:
// 1. Checks for actual token data in multiple possible field formats
// 2. Falls back to educated estimates based on tool type and agent workload
// 3. Flags each entry as "estimated" or actual for transparency
//
// Estimates are based on typical usage patterns and should be reasonably accurate
// for cost tracking purposes, though they may not match actual usage exactly.
...
process.stdin.on('end', () => {
  try {
    ...
    // Try to extract actual token usage from hook data
    // NOTE: Claude Code may not provide actual token counts in PostToolUse hook
    // If unavailable, we fall back to estimates above
    let inputTokens = estimates.input;
    let outputTokens = estimates.output;
    let actualUsage = false;

    // Check for actual usage data in various possible field names
    if (data.usage) {
    ...
    } else if (data.token_count) {
      if (typeof data.token_count.input === 'number') {
        inputTokens = data.token_count.input;
        actualUsage = true;
      }
      if (typeof data.token_count.output === 'number') {
        outputTokens = data.token_count.output;
        actualUsage = true;
      }
    ...
    const task = {
      ...
      estimated: !actualUsage  // Track whether these are estimates
    };
    ...
    usage.current_task = {
      tool_name: toolName,
      model: model,
      cost: calculateSingleTaskCost(model, inputTokens, outputTokens),
      timestamp: new Date().toISOString(),
      estimated: !actualUsage
    };
